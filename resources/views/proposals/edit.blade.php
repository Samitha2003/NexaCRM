            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('proposals.update', $proposal) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('title', $proposal->title) }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="amount" class="block font-medium text-gray-700">Amount ($)</label>
                        <input type="number" name="amount" id="amount" step="0.01" min="0" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('amount', $proposal->amount) }}" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="notes" id="notes" rows="4" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('notes', $proposal->notes) }}</textarea>
                    </div>
