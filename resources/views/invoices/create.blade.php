<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-100 leading-tight">Create Invoice</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('invoices.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="invoice_number" id="invoice_number" class="w-full border-gray-300 rounded-md shadow-sm text-gray-400" value="{{ old('invoice_number') }}" required>
                    </div>
                    
                    <div class="mb-4">
                        <label for="amount" class="block font-medium text-gray-700">Amount ($)</label>
                        <input type="number" name="amount" id="amount" step="0.01" min="0" class="w-full border-gray-300 rounded-md shadow-sm text-gray-400" value="{{ old('amount') }}" required>
                    </div>
